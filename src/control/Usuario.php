    ...
    <div style="background: #f5f7fa; padding: 40px 20px;">
        <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="border-collapse: collapse; mso-table-lspace: 0pt; mso-table-rspace: 0pt;">
            <tr>
                ...
                $mail->send();
                $arr_Respuesta = array('status' => true, 'mensaje' => 'Email de recuperación enviado correctamente');
                
            } catch (Exception $e) {
                $arr_Respuesta = array('status' => false, 'mensaje' => 'Error al enviar el email: ' . $mail->ErrorInfo);
            }
        } else {
            $arr_Respuesta = array('status' => false, 'mensaje' => 'Error al actualizar el token de reset');
        }
    }
    echo json_encode($arr_Respuesta);
}
